<template slot-scope="props">
  <div class="sco_timer">
    <span class="sco_timer--days" v-if="days > 0">{{ days }} days, </span>
    <span class="sco_timer--hours">{{ hours }}</span>
    <span class="sco_timer--separator">:</span>
    <span class="sco_timer--mins">{{ minutes }}</span>
    <span class="sco_timer--separator">:</span>
    <span class="sco_timer--secs">{{ seconds }}</span>
  </div>
</template>
<script>
...
export default {
  ...
  props: {
    ...
    /**
     * Show as a stopwatch or full countdown timer
     */
    mode: {
      type: String,
      default: 'stopwatch' // [full|stopwatch]
    },
  ...
  computed: {
     ...
     */
    hours () {
      var v = Math.floor((this.count % MILLISECONDS_DAY) / MILLISECONDS_HOUR)
      v = (this.leadingZero && v < 10 ? `0${v}` : v)
      return v
    },
     ...
     */
    minutes () {
      var v = Math.floor((this.count % MILLISECONDS_HOUR) / MILLISECONDS_MINUTE)
      v = (this.leadingZero && v < 10 ? `0${v}` : v)
      return v
    },
